      ...
      <template #header>
        <div class="card-header">
          <div class="header-left">
            ...
            <span class="export-meta" v-if="lastExportAt">上次导出：{{ formatTime(lastExportAt) }}</span>
          </div>
          <el-button size="small" @click="exportData" :loading="exportLoading" :disabled="exportLoading" v-if="canExport">
...
const canExport = computed(() => isAdmin.value || item.value.creator_id === currentUserId.value)
...
onMounted(async () => {
    ...
    const cachedExportTime = localStorage.getItem(`item_export_last_at_${id}`)
    if (cachedExportTime) lastExportAt.value = cachedExportTime
    try {
...
async function exportData() {
    ...
    try {
        ...
        const res = await apiClient.get(`/export/item/${id}`, { responseType: 'blob' })
        const url = window.URL.createObjectURL(new Blob([res.data]))
        const link = document.createElement('a')
        link.href = url
        link.setAttribute('download', `item_${id}_${ts}.csv`)
        document.body.appendChild(link)
        link.click()
        document.body.removeChild(link)
        window.URL.revokeObjectURL(url)
        lastExportAt.value = now.toISOString()
        localStorage.setItem(`item_export_last_at_${id}`, lastExportAt.value)
