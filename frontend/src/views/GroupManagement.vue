  ...
  <div class="page-container">
    <div class="sidebar">
      <div class="sidebar-header">
        ...
        <div class="action-buttons">
          ...
          <el-button v-if="isAdmin" class="secondary-btn" @click="importDialogVisible = true">
            <el-icon><Upload /></el-icon> 批量导入
          </el-button>
          <el-button v-if="isAdmin" class="secondary-btn" @click="userMgmtVisible = true">
      ...
      <div class="groups-list">
        ...
        <!-- Personal groups: normal users see only their own; admins see all in a collapsed section. -->
        <div class="group-category">
          <h3 v-if="!isAdmin">我的分组</h3>
          <el-empty v-if="!isAdmin && customGroups.length === 0" description="暂无我的分组" :image-size="60" />
          <div v-if="!isAdmin" class="group-cards">
          ...
          <el-collapse v-else v-model="activeAdminSections" class="admin-user-groups">
            <el-collapse-item title="用户分组" name="user-groups">
              <el-empty v-if="customGroups.length === 0" description="暂无用户分组" :image-size="60" />
              <div v-else class="group-cards">
                <div v-for="group in customGroups" :key="group.id"
                  ...
                  <div class="group-info">
                    <span class="group-name">{{ group.name }}</span>
                    <span class="member-count">{{ group.user_ids.length }} 人 · 创建人 ID {{ group.owner_id }}</span>
                  </div>
                  <div class="card-actions">
                    <el-button link type="primary" @click.stop="openDialog(group)">编辑</el-button>
                    <el-button link type="danger" @click.stop="deleteGroup(group.id)">删除</el-button>
    ...
    <div class="group-detail-panel" v-if="currentGroup">
      <div class="detail-header">
        ...
        <el-button type="primary" v-if="!currentGroup.is_org || isAdmin" @click="openDialog(currentGroup)">编辑分组</el-button>
      </div>
...
async function loadUsers() {
    const uRes = await apiClient.get(isAdmin.value ? '/users' : '/users/assignable')
...
async function saveGroup() {
    try {
        if (editingGroup.value) {
            await apiClient.put(`/groups/${editingGroup.value.id}`, form.value)
...
async function uploadCSV() {
    ...
    try {
        await apiClient.post("/groups/import", formData)
...
.admin-user-groups :deep(.el-collapse-item__header) {
    height: auto;
    padding: 0.5rem;
    color: #64748b;
    background: transparent;
...
.admin-user-groups :deep(.el-collapse-item__wrap) {
    background: transparent;
    border-bottom: 0;
}
