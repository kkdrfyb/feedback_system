...
async def import_groups(
    ...
    for row in reader:
        ...
        if role not in ("admin", "creator", "feedbacker"):
            role = "feedbacker"
...
def get_groups(
    ...
    if current_user.role == "admin":
        ...
        groups = db.query(models.Group).filter(
            (models.Group.is_org == True) | (models.Group.owner_id == current_user.id)
        ).all()
...
def create_group(
    ...
    # 权限检查：非管理员不能创建公开组
    if current_user.role != "admin" and group.is_org:
        group.is_org = False
...
def update_group(
    ...
    # 权限检查
    if current_user.role != "admin" and db_group.owner_id != current_user.id:
    ...
    if current_user.role == "admin" and group.is_org is not None: db_group.is_org = group.is_org
